...
public class UDPService implements NetService {
	...
	public boolean sendMessage(Message msg, DatagramSocket serverSocket,
		...
		if (netConf == null || msg == null || serverSocket == null)
			return false;
		...
		DatagramPacket sendPacket = new DatagramPacket(sendData,
				sendData.length, netConf.getInetAddress(), netConf.getPort());
	...
	/**
	 * It will receive messages, it will block for message. If you prefer a
	 * nonblocked mode, DatagramSocket.setSoTimeout() before pass in
	 * 
	 * @param serverSocket
	...
	public Message receiveMessage(DatagramSocket serverSocket)
		...
		try {
			...
			ByteArrayInputStream objInputBytes = new ByteArrayInputStream(
					recvData);
		...
		} catch (SocketTimeoutException e) { // timeout, only if you set the
												// timeout in socket
			// e.printStackTrace();
			return null;
	...
	public Message recvAckMessage(DatagramSocket serverSocket)
		...
		try {
			...
			sendMessage(ack, serverSocket,
					new NetConfig(clientAddr, clientPort));
